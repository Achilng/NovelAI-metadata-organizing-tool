...
## 当前状态
- 首版工具已完成核心功能、Windows 打包验证、导出去重开关、重复图片目录整理和可选按时间排序。
...
- 扩展去重输出：
  - 有重复组时，在输出包文件夹内创建 `image1`、`image2` 等自动编号文件夹。
  - 将重复组代表图和后续重复图都复制到对应文件夹，不移动原始输入文件。
  - XLSX 新增“重复文件夹”列，代表图所在行记录 `image1/` 等目录名。
- 补充重复目录自动化测试，覆盖正面提示词去重、画师串去重、多个重复组和无重复不创建目录。
- 新增输出防呆：
...
- 新增按时间升序整理开关：
  ...
  - 无法取得时间的图片排在最后，时间列留空。
  - 重复图片文件夹按组内最早图片时间添加前缀，例如 `2026-05-26_103000_image1`。
