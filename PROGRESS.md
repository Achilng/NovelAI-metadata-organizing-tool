...
## 2026-06-11 增量整理强化与首扫提速
- 新增“图片输出”三选一选项（默认复制原图，保持旧行为）：
  - 硬链接：同分区 NTFS 下瞬间完成且不占额外空间；单文件失败自动回退复制，结束时以一条警告汇总回退数量。
  - 不输出图片文件夹：跳过 imageN 和失败目录，仅生成 XLSX，适合输入文件夹固定、依赖“图片路径”列回查原图的增量场景。
- 新增缓存清理：
  - 每次增量运行结束自动修剪缓存，删除已从输入中消失的图片对应记录和缩略图（以磁盘内容为准，连带清理孤儿文件），完成消息报告清理数量。
  - 界面新增“清理缓存”按钮（带确认弹窗，新增 `dialog:allow-ask` 权限），删除输出路径同级整个 `.novelai_metadata_cache` 并报告删除文件数与释放空间。
- 首次扫描性能优化：
  - 去重模式缩略图并行化：改为“并行读元数据 → 顺序去重分组 → 并行生成代表图缩略图”三阶段，分组只依赖元数据，编号和代表图选择保持稳定；代表图缩略图失败时按顺序提升组内重复图为新代表（行为与旧串行版在该罕见场景略有差异，已有回归测试覆盖）。
  - PNG 元数据读取改为 Seek 跳过 IDAT 等像素 chunk，只读文本 chunk；大小上限仅对文本 chunk 生效。
  - 未启用去重时新图片整文件只读一次，元数据解析与缩略图解码共用同一份内存字节。
  - 工作线程上限从 8 提高到跟随 CPU 可用并行度（上限 32）。
  - 目录扫描每个文件只调用一次元数据（原来 2~3 次 stat）；进度事件按 100ms 节流，并行阶段现在能实时上报进度（原先要等全部完成才刷新）。
  - 元数据失败的图片不再额外生成缩略图（旧版会生成但从不使用）。
- 新增 6 个测试：硬链接输出、不输出图片文件夹、缓存修剪、清空缓存目录、代表图缩略图失败提升、Seek 跳过大型非文本 chunk。
- 验证 `cargo check` 无警告，`cargo test` 43 个测试全部通过，`npm.cmd run build` 通过。
...
## 当前状态
- 工具已完成图片元数据整理、增量缓存（含自动修剪与手动清理）、去重目录输出（复制/硬链接/不输出三种方式）、XLSX 转智绘姬 JSON，以及智绘姬 JSON 去重功能。
...
## 后续建议
- 使用用户实际 NovelAI PNG / `.zip` / `.7z` / `.rar` 样例做人工验收（尤其是上万张图的增量 + 硬链接/不输出场景）。
- 视需要补充取消按钮、拖放输入等增强功能。

## 记录
- 2026-06-11：新增图片输出三选一（复制/硬链接/不输出）、缓存自动修剪与清理按钮、去重缩略图并行化和元数据 Seek 读取等首扫提速；验证 `cargo test` 43 个测试全部通过，`npm.cmd run build` 通过。
