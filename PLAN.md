...
|---|---|---|
| A | 图片 | 嵌入缩略图 |
| B | 图片路径 | 文件夹/单 PNG 输入时为原图路径；压缩包输入时为压缩包路径和包内路径 |
| C | 时间 | 仅启用按时间排序时存在，图片文件创建时间 |
| D | 正向提示词 | Positive prompt |
| E | 负向提示词 | Negative prompt |
| F | 画师串 | 提取出的画师片段 |
| G | 重复文件夹 | 该代表图对应的自动编号重复目录，非重复时为空 |
